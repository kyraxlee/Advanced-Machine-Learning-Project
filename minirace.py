...
# -*- coding: utf-8 -*-
"""
Last modified on Mon Sep 29 13:08 2025 
...
class Minirace:
    ...
    def render(self, text = True, reward = None, cm = plt.cm.bone_r, f = None):
        ...
        if text:
            ...
            if reward is not None:
                self._ax.set_title(f'Reward: {reward}', loc='right')
            self._ax.axis("off")
            self._ax.imshow(pix, origin='lower' , cmap=cm)
            if f is not None:
    ...
    for i_episode in count(1):
        ...
        for t in range(1, 10000):  # Don't infinite loop while learning
            ...
            if render and rendernow:
                therace.render(text=True,reward = ep_reward)
